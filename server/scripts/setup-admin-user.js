// Versão JavaScript do script de configuração do usuário administrador
...
// Para resolver caminhos em módulos ES
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Para importar módulos como CommonJS se necessário
  ...
  try {
    ...
    if (existingUser) {
      ...
      if (existingUser.role === 'admin') {
        console.log('Usuário administrador já configurado:', ADMIN_USERNAME);
        return;
      }
      
      // Atualizar para admin se não for
      await storage.updateUser(existingUser.id, { role: 'admin' });
      console.log('Usuário promovido para administrador:', ADMIN_USERNAME);
      return;
    }
...
IMPORTANTE: Guarde este arquivo em local seguro e depois delete-o!
      ...
      console.log(`Senha gerada e salva em: ${passwordFile}`);
    }
    ...
    if (isGeneratedPassword) {
      console.log('Usuário administrador criado com senha gerada:', ADMIN_USERNAME);
      console.log('Senha salva em: admin-password.txt');
    } else {
      console.log('Usuário administrador criado com senha da variável de ambiente:', ADMIN_USERNAME);
    }
